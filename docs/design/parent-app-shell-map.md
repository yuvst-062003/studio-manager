...
| טכניקות | `#/techniques` | **new** — the judo technique library, merged in from a parallel branch |
...
**The fifth tab arrived after this map was written**, from the branch that built the
technique library while this one rewrote the shell. Its own design (§6 of
`docs/superpowers/specs/2026-09-05-judo-technique-library-design.md`) had the owner's
decision that the library belongs in the bar rather than folded into a screen, and it goes
fourth so home, shop and updates keep the positions a thumb already knows. That design's
wiring note also asked for a `NAV` drawer entry, which no longer exists — see the note in
`features/techniques/index.ts` for what the merge made stale and why.

Nothing in the bar hardcoded four, so the cost was five records gaining a row each.
Measured at 360px with all five in place: the labels total 190px of the 312px between the
bar's own padding, every one on a single line, none clipped, and the page does not scroll
sideways — `docs/screenshots/technique-library/step-5/`.
...
|---|---|---|
| 1 | the shell — four tabs, no drawer | ✅ mounted (five since the technique library merged) |
