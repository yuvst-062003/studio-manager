...
| 19 | **The uPay receipt link** — **CLOSED 2026-09-11 by a live ₪1 payment on production: no button, ever** | D6 asked for one look at uPay. Two facts settle it, and the second one is now observed rather than inferred. **A receipt URL exists and is shareable** — uPay mails the customer `https://app.upay.co.il/API6/s.php?m=<token>`, which needs no login, so a manager could open one. **But the token is not an id**: decoded it is double-base64 around exactly 16 bytes of binary (`228bc546…`), one AES block, i.e. ciphertext. Nothing in `external_payment_ref`, `transactionid` or anywhere else we store can construct it — only uPay can. So everything hung on whether uPay *hands us* the link, and on **2026-09-11 the first real callback in this system's history arrived and said no.** A ₪1 manual charge (`4cf4a41a-0ad8-4bde-84b3-161c79cfb0c1`) was paid on production with a real card; `upay_ipn_record.raw_query` was then read for parameter **names** only, values left alone. **The payload carries 31 keys — exactly the 31 `app/integrations/upay/callback.py` already parses.** Nothing extra arrived, nothing expected was missing, and no key contains `invoice`, `receipt`, `doc`, `link`, `url` or `pdf`. The form does send `createinvoiceandreceipt=1` and uPay does issue a קבלה — it simply never tells us where it is. **The button is impossible and this row is permanent.** The paid row stays exactly as D6 specified: the typed receipt number as plain text, no control beside it. **No code changed for this checkpoint, in either direction.** |
