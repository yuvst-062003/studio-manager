...
| 19 | **The uPay receipt link** — **ANSWERED 2026-09-11: no button, and here is why** | D6 asked for one look at uPay. The owner supplied a real receipt email, which settles more than the original question did. **A receipt URL exists and is shareable** — uPay mails the customer `https://app.upay.co.il/API6/s.php?m=<token>`, and it needs no login, so a manager COULD open one. **But the token is not an id.** Decoded it is double-base64 around exactly 16 bytes of binary (`228bc546…`) — one AES block, i.e. ciphertext. Nothing in `external_payment_ref`, `transactionid` or anywhere else we store can construct it; only uPay can. **So the remaining question is whether uPay HANDS us the link**, and that cannot be answered today: `upay_ipn_record` holds **zero rows in production and zero in staging** (checked over `railway ssh` on 2026-09-11), so no real callback has ever arrived. Our parser knows 40 IPN fields and none is a document link, but that only proves what we parse, not what uPay sends. **What closes it:** the first real payment through this system. `upay_ipn_record.raw_query` keeps the callback verbatim, so one query over its parameter NAMES (values are not needed and should not be read — the row holds a cardholder name and last four) says whether a receipt link is in there. If it is, store it and the button is trivial. If it is not, the button is impossible and this row becomes permanent. Until then the paid row stays exactly as D6 specified: the typed receipt number as plain text, no control beside it. **No code changed for this checkpoint, in either direction.** |
