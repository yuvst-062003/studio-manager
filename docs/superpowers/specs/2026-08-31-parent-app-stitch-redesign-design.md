...
| **Screen 5 · payments** — 2026-09-01 | **No Stitch attempt — the owner asked for Claude Design directly for this pass.** Three arrangements were drawn against the code's resolved values (`[data-surface="outward"]`, `Alert`, `MoneyDisplay`, `Card`), on one axis: how the amount owed and the way to clear it become one movement. **The owner picked B, total-first with the routes behind a picker** — but B's literal navigation could not ship: §5.10 and `ParentBilling.test.tsx` both hold that all four routes stay visible together, which a picker unmounting three of them directly contradicts. What shipped carries B's spirit inside that constraint — the total leads the ledger as a headline, and the הוראת קבע warning moved from inside the card route's own panel (where a family paying cash or cheque never saw it) to the top of the whole screen, with per-child amounts reused from `standingOrderLinks`. All four routes stay exactly as expanded as they were. | Candidate **A** (every route expanded on one long card — today's shipped shape, and the reason B was picked over keeping it). Candidate **C** (no month-count chips; each month a card to tick, route chosen once at the top) — rejected as more repeated chrome for a family several months behind. |
| **Screen 8 · profile** — 2026-09-01 | **Stitch produced nothing for this pass either.** `generate_screen_from_text` against the same project, with the Gladiator system and every fixed constraint, timed out; five minutes of `list_screens` showed the project still holding only its `DESIGN.md` screen. The pass fell back to Claude Design, which [`docs/design/prompts/parent-screens.md`](../../design/prompts/parent-screens.md) already names as the alternative, drawing against the code's resolved values (`[data-surface="outward"]` navy `#003874`, card 14px, button 2px, the 46×27 switch, chip padding 6/11) and the shipped Hebrew of `he/people.ts` and `he/common.ts`. Three arrangements went to the owner on one axis — **where the destructive action lives** — and **the owner picked B, the layered settings list**: every row states its current value and leads to where it is changed, and leaving the club is not on the screen at all. | Candidate **A** (one page, edit in place, with leaving at the bottom under an *אזור רגיש* heading — rejected as detaching the act from the child it ends) and candidate **C** (opens ready to edit, leaving behind a per-child *ניהול* expand — rejected as the longest page and the shortest path to a permanent act). Also cut from within B, by the owner: the **delete-my-data** control. `purge_subject_data` raises until it is built, so every request would end `failed`; a parent asks the manager instead, which is what earns the club's telephone its row. |
...
| 5 | **Payments** — **shipped 2026-09-01** | `features/billing/PaymentsSection.tsx`, `PaymentHistorySection.tsx` | **Settled: the warning.** The הוראת קבע double-charge warning now leads the screen, before the ledger, with the per-child amounts it used to lack — see *Screen 5* in Provenance. **Left as they were:** the two stacked segmented pickers, and D9.3's receipt affordance (shipped ahead of this pass, in `PaymentHistorySection.tsx`, untouched by it). |
...
| 8 | **Profile** — **shipped 2026-09-01** | `features/people/GuardianSettings.tsx` (new), `ProfileSection.tsx` | Was titled **חניכים** with the destructive **עזיבת המועדון** as its only per-child affordance. Now the settings-list arrangement (candidate B): the guardian's own name, email and phone — editable, which needed a new `PATCH /me/profile` — plus language, theme, a notifications switch that travels both ways (`DELETE /push-tokens`), the payment method, the children each leading to their own card, the club's address and telephone, and the data export. **Leaving is not on this screen**; it belongs to the child's card, which screen 4 owns. Deletion is deliberately absent while `purge_subject_data` is unbuilt. |
