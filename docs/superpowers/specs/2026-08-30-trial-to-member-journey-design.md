...
3. `student.price_plan_id` from **weekly volume across the chosen groups** — the
   same rule `register` and `add_child` use. Exactly one live plan may match; zero
   or two leave the student unpriced, which defect 3 makes visible.
...
* A manager conversion raises a first charge, so the payment step has something to
  show.
* A student whose groups match no plan appears in the unpriced list.
* `is_invite_only` is refused as not-found on the join path, as on every other.

## Out of scope

* Changing how the price is derived. Weekly volume decides it, on every path.
* Any parent-facing way to choose a price directly. The groups are the choice.
