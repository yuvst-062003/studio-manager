...
export type HealthStatusSummaryOut = components['schemas']['HealthStatusSummaryOut']
export type HealthFormTemplateOut = components['schemas']['HealthFormTemplateOut']
export type HealthTemplatePublishedOut = components['schemas']['HealthTemplatePublishedOut']
...
export function makeHealthClient(fetcher: Fetcher) {
  return {
    ...
    summary: (filter: DocumentFilter = 'all'): Promise<HealthStatusSummaryOut[]> =>
      fetcher(
        filter === 'all'
    ...
    template: (id: string): Promise<HealthFormTemplateOut> =>
      fetcher(`/api/v1/health-templates/${id}`).then(json<HealthFormTemplateOut>),

    /** D11's edit. Saves a DRAFT — nothing a parent signs moves until `publish`. */
    saveDraft: (id: string, schema: EditableSchema): Promise<HealthFormTemplateOut> =>
