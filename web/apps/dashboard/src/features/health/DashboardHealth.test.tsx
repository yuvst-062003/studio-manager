...
function makeClient(
  over: Partial<DashboardHealthClient> = {},
): DashboardHealthClient {
  return {
    summary: vi.fn().mockResolvedValue(ROWS),
    remind: vi
      .fn()
      .mockResolvedValue({ last_reminder_sent_at: "2026-11-03T12:00:00Z" }),
    templates: vi
      .fn()
      .mockResolvedValue({
...
describe("DocumentsScreen", () => {
  ...
  it("shows a reminder button on a row that owes a declaration, and reports the send", async () => {
    ...
    expect(
      await screen.findByText(t("he", "health.reminder.sent")),
    ).toBeInTheDocument();
  });
  ...
  it("renders the goal state when everything is filed", async () => {
    ...
    render(
      <DocumentsScreen
        client={makeClient({ summary: vi.fn().mockResolvedValue([]) })}
        locale="he"
      />,
    );
    expect(
      await screen.findByText(t("he", "health.documents.empty")),
    ).toBeInTheDocument();
  });
  ...
  it("the group request counts exactly the rows on screen that still owe something", () => {
    ...
    // disagree with them.
    expect(chaseable(ROWS).map((row) => row.student_id)).toEqual([
      "st1",
      "st3",
    ]);
  });
  ...
  it("renders in English too", async () => {
    ...
    expect(
      await screen.findByText(t("en", "health.documents.title")),
    ).toBeInTheDocument();
  });
});
...
describe("TemplateEditor", () => {
  it("renders D11's caveat unconditionally, before anything can be changed", async () => {
    ...
    expect(
      await screen.findByText(t("he", "health.template.disclaimer")),
    ).toBeInTheDocument();
  });
  ...
  it("saves a draft rather than editing what parents are signing", async () => {
    ...
    await userEvent.click(
      screen.getByRole("button", { name: t("he", "health.template.save") }),
    );
    expect(client.saveDraft).toHaveBeenCalled();
    expect(
      await screen.findByText(t("he", "health.template.saved")),
    ).toBeInTheDocument();
  });
  ...
  it("reports how many declarations a publish re-derived", async () => {
    ...
    await screen.findByDisplayValue("האם יש אסתמה?");
    await userEvent.click(
      screen.getByRole("button", { name: t("he", "health.template.publish") }),
    );
...
describe("the schema edits, without a DOM", () => {
  ...
  it("a flag can be turned on and off explicitly", () => {
    expect(
      withFlag(SCHEMA, "asthma", false).sections[0]!.questions[0]!.flag,
    ).toBe(false);
    expect(
      withFlag(SCHEMA, "asthma", true).sections[0]!.questions[0]!.flag,
    ).toBe(true);
  });
