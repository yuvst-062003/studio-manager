...
export function withNewQuestion(
  schema: EditableSchema,
  sectionId: string,
): EditableSchema {
...
export function TemplateEditor({ locale, client }: TemplateEditorProps) {
  ...
  return (
    <section aria-labelledby="template-title">
      ...
      >
        {bundled
          ? t(locale, "health.template.editingBundled")
          : t(locale, "health.template.editingYours")}
      </p>
          ...
          {section.questions.map((question) => (
            <div key={question.id} style={rowStyle}>
              <TextField
                ...
                onChange={(event) =>
                  change(
                    withQuestionLabel(schema, question.id, event.target.value),
                  )
                }
              ...
              <label>
                <input
                  ...
                  onChange={(event) =>
                    change(withFlag(schema, question.id, event.target.checked))
                  }
                  type="checkbox"
                />
