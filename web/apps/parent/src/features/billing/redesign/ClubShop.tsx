...
export function ClubShop({ locale }: { locale: Locale }) {
  ...
  const payByCard = useCallback(async () => {
    ...
    try {
      // **The retry reuses the order it already opened**, and `orderRefFor` is where that
      // decision lives for every card route in the app. `OrderService.create` refuses a
      // charge one of the payer's own pending orders holds for `REPLACE_GRACE_MINUTES`, so
      // a second `createOrder` over the same charges 409s and the parent reaches no payment
      // page at all.
      //
      // This used to be a local `pendingRef`, which covered a failed FORM fetch and nothing
      // else: it was cleared the moment the overlay opened, so the commonest way to leave a
      // checkout — the X on the overlay — put the parent straight back into that 409, over
      // an order the club had already placed. `orderRefFor` asks the server, which does not
      // forget. One payment, no prepaid months: an item order is a one-off, not a
      // subscription.
      const publicRef = await billing.orderRefFor([...chargeIds], 1, 0)
