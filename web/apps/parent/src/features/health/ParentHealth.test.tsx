...
describe("SignaturePad", () => {
  beforeEach(() => {
    ...
    (globalThis as unknown as { __strokes: typeof strokes }).__strokes =
      strokes;
  ...
  it("the canvas is direction-isolated, so a stroke cannot be flipped by an RTL ancestor", () => {
    ...
    expect(screen.getByTestId("signature-canvas").getAttribute("dir")).toBe(
      "ltr",
    );
  });
  ...
  it('a pointer path drawn left to right produces increasing canvas x, inside dir="rtl"', () => {
    ...
    canvasProto.getBoundingClientRect = vi.fn(
    ...
    const strokes = (globalThis as unknown as { __strokes: { x: number }[] })
      .__strokes;
    strokes.length = 0;
...
describe("the question rules", () => {
  ...
  it("an invisible question is never required", () => {
    expect(
      unansweredRequired(SCHEMA, { asthma: false, allergy: false }),
    ).toEqual([]);
  });
});
...
describe("DeclarationForm", () => {
  ...
  it("refuses to submit an answered form with no signature", async () => {
    ...
    expect(screen.getByRole("alert")).toHaveTextContent(
      t("he", "health.declaration.signatureRequired"),
    );
  });
  ...
  it("says the declaration never expires rather than showing a validity date", async () => {
    ...
    expect(
      await screen.findByText(t("he", "health.declaration.noExpiry")),
    ).toBeInTheDocument();
  });
  ...
  it("a missing declaration blocks the app entirely — the children are not rendered at all", async () => {
    ...
    render(
      <HealthGate
        ...
        locale="he"
        students={[
          { id: "st1", display_name: "נועה לוי", health_status: "missing" },
        ]}
      >
        {app}
      </HealthGate>,
