...
//
// **The dark inversion is deliberate and local.** `StaffShell`'s own ground is light
// (`#f3f6fb`); this screen paints its own `#090d16` ground over the `<main>` it sits in,
// exactly as the prototype's root div does, and `StaffTabBar` already watches for
// `active === 'timer'` to follow suit — see that file's own header.
//
...
export function TimerScreen({ locale }: { locale: Locale }) {
  ...
  return (
    <section
      ...
      className="w-full flex-1 flex flex-col bg-[#090d16] text-slate-100 select-none"
    >
      <header className="w-full px-4 py-2.5 flex items-center justify-between border-b border-slate-800/80 bg-[#090d16] sticky top-0 z-30">
        ...
        <button
          ...
          className="px-2.5 py-1.5 rounded-xl bg-slate-800/90 border border-slate-700/80 flex items-center gap-1.5 text-xs text-slate-200 active:scale-95 transition-all shadow-xs"
        >
          <span
            aria-hidden="true"
      ...
      <div className="px-4 py-3 flex flex-col gap-3">
        {feedbackToast ? (
          <div
            role="status"
            aria-live="polite"
            className="fixed top-14 start-1/2 -translate-x-1/2 z-50 bg-slate-900/95 border border-amber-500/80 text-white text-xs font-bold px-4 py-2.5 rounded-2xl shadow-2xl flex items-center gap-2"
          >
              ...
              <div
                ...
                className={`relative shrink-0 flex items-center rounded-full text-xs font-medium border transition-all ${
                  isActive
                    ...
                    : custom
                      ? 'bg-slate-800/90 text-amber-200 border-amber-500/30 hover:border-amber-400/50'
              ...
              >
                ...
                {isCustomPreset(preset) ? (
                  <button
                    ...
                    className="ps-1 pe-2 text-slate-400 hover:text-rose-400 transition-colors"
                  >
        ...
        <section
          className={`relative rounded-3xl p-4 border border-slate-800 bg-[#111827] flex flex-col items-center justify-center transition-all duration-300 shadow-xl ${PHASE_GLOW[engine.phase]}`}
        >
          <div className="flex items-center justify-between w-full mb-1">
            ...
            <div className="text-end font-mono">
              ...
              <span className="text-xs font-bold text-slate-200">{formatTime(engine.totalRemainingSeconds)}</span>
            </div>
          </div>

          {/* 3-2-1 countdown pips. */}
          <div className="flex items-center justify-center gap-3 w-full my-1">
            <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-slate-900/80 border border-slate-800 shadow-inner">
              <span className="text-[10px] text-slate-400 font-bold me-1 tracking-wider uppercase flex items-center gap-1">
              ...
              <span className="text-[9px] font-semibold text-slate-500 ms-1">
          ...
          <div className="relative w-56 h-56 flex items-center justify-center my-1">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center text-center pointer-events-none">
              <div
                ...
                className="text-6xl font-black font-mono tracking-tight text-white select-none drop-shadow-md"
              >
          ...
          {/* Round / set counters. */}
          <div className="grid grid-cols-2 gap-2.5 w-full mt-1 pt-3 border-t border-slate-800/80">
            <div className="bg-slate-800/60 rounded-2xl p-2 text-center border border-slate-700/50">
              <span className="text-[11px] text-slate-400 font-medium block">{t(locale, 'timer.roundLabel')}</span>
              <div className="flex items-baseline justify-center gap-1 mt-0.5 font-mono">
                <span data-testid="timer-round" className="text-2xl font-black text-white">
                  {engine.round}
                </span>
                <span className="text-slate-500 font-bold">/</span>
                <span className="text-sm font-bold text-slate-400">{engine.settings.rounds}</span>
              </div>
            </div>

            <div className="bg-slate-800/60 rounded-2xl p-2 text-center border border-slate-700/50">
              <span className="text-[11px] text-slate-400 font-medium block">{t(locale, 'timer.setLabel')}</span>
              <div className="flex items-baseline justify-center gap-1 mt-0.5 font-mono">
                <span data-testid="timer-set" className="text-2xl font-black text-amber-400">
                  {engine.set}
                </span>
                <span className="text-slate-500 font-bold">/</span>
                <span className="text-sm font-bold text-slate-400">{engine.settings.sets}</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2.5 w-full mt-3.5">
            <button
              ...
              className="w-13 h-13 min-w-[50px] p-3 rounded-2xl bg-slate-800 hover:bg-slate-700 active:scale-90 text-slate-300 flex items-center justify-center transition-all border border-slate-700 shadow-md"
            >
        ...
        <section className="bg-[#111827] rounded-3xl p-3.5 border border-slate-800 flex flex-col gap-2.5 shadow-lg">
        ...
        <div className="rounded-2xl bg-blue-950/30 border border-blue-900/40 p-3 flex items-start gap-2 text-xs text-blue-200">
          <Sparkles className="w-4 h-4 text-blue-400 shrink-0 mt-0.5" aria-hidden="true" />
          <div className="leading-relaxed">
            <span className="text-white font-bold block mb-0.5">{t(locale, 'timer.tip.heading')}</span>
...
function AdjustTile({
  ...
  return (
    <div className={`bg-[#182232] p-2.5 rounded-2xl border flex flex-col justify-between ${ACCENT[accent]}`}>
      ...
      <div className="flex items-center justify-between mt-1.5">
        <button
          ...
          className="w-8 h-8 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 flex items-center justify-center font-bold text-lg border border-slate-700/60 active:scale-90"
        >
          -
        </button>
        <span data-testid={`${testId}-value`} className="text-lg font-black font-mono text-white">
          {value}
        </span>
        <button
          type="button"
...
function SmallAdjustTile({
  ...
  return (
    <div className="bg-slate-800/40 p-2 rounded-xl flex items-center justify-between border border-slate-700/50">
      <div>
        <span className="text-[10px] font-semibold text-slate-300 block">{label}</span>
      ...
      <div className="flex items-center gap-1">
        <button
          ...
          onClick={onDecrease}
          aria-label={decreaseAria}
          className="w-6 h-6 rounded-lg bg-slate-700 text-slate-200 text-xs font-bold active:scale-90"
        >
          -
        </button>
  ...
  return (
    ...
    >
      <div
        ...
        className="bg-[#131b2b] border border-slate-700 rounded-3xl p-5 w-full max-w-sm text-slate-100 shadow-2xl relative space-y-4"
      ...
      >
        <div className="flex items-center justify-between border-b border-slate-800 pb-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-sm font-black text-white">{t(locale, 'timer.modal.heading')}</h3>
              <p className="text-[11px] text-slate-400">{t(locale, 'timer.modal.subheading')}</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <div className="bg-slate-900/90 rounded-2xl p-3 border border-slate-800 space-y-2">
          ...
          <div className="grid grid-cols-3 gap-2 text-center text-xs font-mono">
            <div className="bg-slate-800/80 p-1.5 rounded-xl border border-rose-500/20">
              <span className="text-[9px] text-rose-300 block">{t(locale, 'timer.modal.workShort')}</span>
              <span className="font-bold text-white">{formatTime(settings.workTime)}</span>
            </div>
            <div className="bg-slate-800/80 p-1.5 rounded-xl border border-emerald-500/20">
              <span className="text-[9px] text-emerald-300 block">{t(locale, 'timer.modal.restShort')}</span>
              <span className="font-bold text-white">{formatTime(settings.restTime)}</span>
            </div>
            <div className="bg-slate-800/80 p-1.5 rounded-xl border border-blue-500/20">
              <span className="text-[9px] text-blue-300 block">{t(locale, 'timer.modal.roundsShort')}</span>
              <span className="font-bold text-white">{settings.rounds}</span>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between text-[11px] text-slate-400 pt-1 border-t border-slate-800/80">
            <span>
              ...
              <strong className="text-white font-mono">{settings.sets}</strong>
            </span>
            <span>
        ...
        <div>
          <label htmlFor="timer-preset-name" className="block text-xs font-bold text-slate-300 mb-1.5">
          ...
          <input
            ...
            autoFocus
            className="w-full h-11 px-3.5 bg-slate-900 border border-slate-700 rounded-xl text-sm text-white placeholder-slate-500 focus:outline-none focus:border-amber-400 focus:ring-1 focus:ring-amber-400"
            onKeyDown={(event) => {
          ...
          <div className="flex flex-wrap gap-1.5">
            {QUICK_SUGGESTION_KEYS.map((key) => (
              <button
                ...
                className="text-[10px] font-semibold px-2 py-1 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 active:scale-95 transition-all"
              >
                {t(locale, key)}
              </button>
            ))}
        ...
        <div className="grid grid-cols-2 gap-2 pt-2 border-t border-slate-800">
          <button
            type="button"
            onClick={onClose}
            className="h-10 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 font-bold text-xs active:scale-95 transition-all"
          >
