...
describe('HealthBadge', () => {
  it('nothing it renders is disabled — §5.5 blocks nothing on the mat', () => {
    ...
    const { container } = render(
      <HealthBadge flags={NO_FLAGS} locale="he" onRemind={vi.fn()} status="missing" studentId="st1" />,
    )
  ...
  it('a missing declaration shows the ⚠ AND the hint that says marking is still allowed', () => {
    render(<HealthBadge flags={NO_FLAGS} locale="he" status="missing" studentId="st1" />)
  ...
  it('one-tap reminder calls back exactly once, with the student it is about', async () => {
    ...
    await userEvent.click(screen.getByRole('button', { name: t('he', 'health.reminder.send') }))
  ...
  it('renders one chip per raised flag and none for the rest', () => {
    ...
    expect(screen.getByText(t('he', 'health.flag.medication'))).toBeInTheDocument()
  ...
  it('a flag the namespace cannot name renders nothing rather than a blank chip', () => {
    ...
    const { container } = render(
      <HealthBadge flags={{ vertigo: true }} locale="he" status="signed" studentId="st1" />,
    )
  ...
  it('never renders a flag value, only its fixed label', () => {
    ...
    render(<HealthBadge flags={{ asthma: true }} locale="he" status="signed" studentId="st1" />)
  ...
  it('tells the coach the full record is not theirs to open', () => {
    ...
    render(<HealthBadge flags={{ asthma: true }} locale="he" status="signed" studentId="st1" />)
    expect(screen.getByText(t('he', 'health.flag.detailsRestricted'))).toBeInTheDocument()
  })

  it('a trial declaration is labelled as one', () => {
    render(<HealthBadge flags={NO_FLAGS} locale="he" status="trial_signed" studentId="st1" />)
    expect(screen.getByText(t('he', 'health.badge.trialSigned'))).toBeInTheDocument()
  })

  it('a signed declaration with no flags is quiet', () => {
    const { container } = render(<HealthBadge flags={NO_FLAGS} locale="he" status="signed" studentId="st1" />)
    expect(container.querySelectorAll('.studio-chip')).toHaveLength(0)
  })

  it('renders in English too', () => {
    render(<HealthBadge flags={NO_FLAGS} locale="en" status="missing" studentId="st1" />)
    expect(screen.getByText(t('en', 'health.badge.missingHint'))).toBeInTheDocument()
  })
  ...
  it('labelledFlags returns only true, labelled flags, in a fixed order', () => {
    expect(labelledFlags({ other: true, asthma: true, vertigo: true, heart: false })).toEqual([
      'asthma',
      'other',
    ])
  })
