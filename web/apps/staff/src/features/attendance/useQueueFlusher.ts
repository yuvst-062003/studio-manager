...
import { apiFetch, flushNow, refresh } from '@studio/core'
...
export function useQueueFlusher(currentPersonId: string | null): void {
  useEffect(() => {
    ...
    // Three triggers, because one is never enough. The `online` event fires when the OS
    // says a network exists, which §10.1 is explicit is not the same as the API being
    // reachable; the timer covers the case where it lied, and the mount covers a queue that
    // was left behind by a previous launch.
    drain()
